...
class HubSpotTracker {
  ...
  async submitLead(data: LeadData): Promise<boolean> {
    try {
      ...
      if (enrichedData.company) formData.append('company', enrichedData.company);
      if (enrichedData.utm_source) formData.append('utm_source', enrichedData.utm_source);
      if (enrichedData.utm_medium) formData.append('utm_medium', enrichedData.utm_medium);
      if (enrichedData.utm_campaign) formData.append('utm_campaign', enrichedData.utm_campaign);
      if (enrichedData.utm_content) formData.append('utm_content', enrichedData.utm_content);
      if (enrichedData.utm_term) formData.append('utm_term', enrichedData.utm_term);

      const formId = data.form_type === 'Form 1' ? this.form1Id : this.form2Id;
      const response = await fetch(`https://api.hsforms.com/submissions/v3/integration/submit/${this.portalId}/${formId}`, {
        method: 'POST',
        body: formData,
      });
...
export const initializeHubSpot = (portalId: string, form1Id: string, form2Id: string): HubSpotTracker => {
  if (!hubspotTracker) {
